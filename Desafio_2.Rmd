...
Analisando histogramas, chega-se à conclusão que os valores do prêmio têm comportamento assimétrico, com maior variabilidade, já que há pontos cujos valores de seguro são muito altos (outliers). Já o log do prêmio apresenta comportamento se assemelhando à distribuição normal, diminuindo a variabiidade e estibilizando um pouco a incerteza. 
Dessa forma, se escolhe modelar o logaritmo do prêmio.
...
Interpretando os gráficos gerados a seguir, percebe-se que a maioria das observações relativas às residências dos segurados se encontram na faixa nordeste do município (ao norte e ao leste) e que os valores do logaritmo do prêmio pago pelo segurado estão concentrados entre 7 e 8, em média, e aparentam apresentar distribuição normal.
...
```{r fig.height = 5, include = TRUE, fig.weight = 12}
par(mfrow=c(1,1))
...
Analisando o semivariograma empírico e a tendência estimada por máxima verossimilhança, verifica-se que, após testar os modelos da família matern, esférica, exponencial e gaussiana, optou-se pela utilização do modelo gaussiano. 

O alcance encontrado foi bem pequeno, mesmo indicando um processo razoavelmente suave ao longo do espaço. Assim, já para pequenas distâncias, não há tanta dependência espacial.
...
* confirmando o alcance pequeno observado anteriormente, na região escura, em pequenas distâncias já não há dependências expressivas entre os dados;
...
Pelo histograma a seguir, usando uma janela de 2.500 pontos preditos, percebe-se que o y ajustado parece ter distribuição normal em torno de 7 e 8.
...
Em comparação, os maiores prêmios estão concentrados em regiões que cruzam vias expressas, divisas de municípios e outras áreas espalhadas pela cidade.
...
O que não ocorre na parte mais ao leste e ao norte do mapa, em que existe mais certeza encontrada, ou seja, muitos pontos ao redor e variância de predição pequena.
...
Por fim, é possível concluir que, através dos métodos de interpolação ponderada pelo inverso da distância e krigagem:
